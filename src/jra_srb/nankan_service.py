...
class NankanService:
    def __init__(self, provider: BaseNankanProvider | None = None, cache: TTLCache | None = None) -> None:
        ...
        if parsed.get("weather") is None or parsed.get("track_condition") is None:
            for key, value in (await self._meeting_conditions_for_race(race_id, refresh=refresh)).items():
                if parsed.get(key) is None and value is not None:
                    parsed[key] = value
        result = RaceCard(race_id=race_id, fetched_at=datetime.now(UTC), source=page.source, **parsed)
        self.cache.set(cache_key, result, ttl_seconds=300)
        return result
    ...
    async def get_leading_jockeys(
        ...
        cache_key = f"nankan:leading:jockeys:{condition_code}"
        cached = self.cache.get(cache_key)
        if cached is not None and not refresh:
            return cached.model_copy(update={"cache_hit": True})
        fallback = False
        ...
        result = NankanLeadingJockeyPage(
    ...
    async def get_race_result(self, race_id: str, refresh: bool = False) -> RaceResult:
        ...
        result = RaceResult(race_id=race_id, fetched_at=datetime.now(UTC), source=page.source, **parsed)
        self.cache.set(cache_key, result, ttl_seconds=3600)
        return result
    ...
    async def get_race_odds(
        ...
        for current in requested:
            ...
            cached = self.cache.get(cache_key)
            if cached is not None and not refresh:
                odds_map[current] = await self._complete_win_odds(race_id, cached, refresh=refresh) if current == "win" else cached
                continue
            ...
            self.cache.set(cache_key, entries, ttl_seconds=60)
    ...
    async def _meeting_id(self, target_date: date, course: str, refresh: bool) -> str:
        ...
        if cached is None or refresh:
            ...
            self.cache.set(cache_key, meeting_ids, ttl_seconds=3600)
        else:
            meeting_ids = cached
    ...
    async def _meeting_conditions_for_race(self, race_id: str, refresh: bool) -> dict[str, str | None]:
        ...
        self.cache.set(cache_key, conditions, ttl_seconds=300)
        return conditions
