...
def _last_corner(value: Any, field_size: int) -> float | None:
    ...
    return (
        int(parts[-1]) / field_size
        if parts and parts[-1].isdigit() and field_size
        else None
    )
...
def load_data(db: Path):
    ...
    for race_id, combination, exact, lower, upper in wide_rows:
        ...
        try:
            price = (
                float(exact) if exact is not None else (float(lower) + float(upper)) / 2
            )
        except (TypeError, ValueError):
            continue
        if pair:
...
def build_runner_rows(raw: list[dict[str, Any]]) -> list[RunnerRow]:
    ...
    for race_rows in races.values():
        ...
        closing = sorted(
            ...
            if row.get("final_3f") is not None
        )
        for row in race_rows:
            ...
            row["position_gain"] = (
                row["corner_ratio"] - row["finish_ratio"]
                if row["corner_ratio"] is not None
                else None
            )
            row["final3f_percentile"] = (
                closing.index(float(row["final_3f"])) / max(len(closing) - 1, 1)
                if closing and row.get("final_3f") is not None
                else None
            )
    ...
    for race_date in sorted(by_date):
        for race_id in sorted(by_date[race_date]):
            for row in races[race_id]:
                ...
                features = (
                    float(last["rank"] - last["popularity"])
                    if last and last.get("popularity")
                    else None,
                    ...
                    if any(item.get("popularity") for item in recent)
                    else None,
                    mean(
                        item["final3f_percentile"]
                        for item in recent
                        if item.get("final3f_percentile") is not None
                    )
                    if any(
                        item.get("final3f_percentile") is not None for item in recent
                    )
                    else None,
                    mean(
                        item["position_gain"]
                        for item in recent
                        if item.get("position_gain") is not None
                    )
                    if any(item.get("position_gain") is not None for item in recent)
                    else None,
                    float(
                        (
                            date.fromisoformat(race_date)
                            - date.fromisoformat(last["race_date"])
                        ).days
                    )
                    if last
                    else None,
                    ...
                    float(
                        abs(
                            _distance(last.get("distance"))
                            - _distance(row.get("distance"))
                        )
                    )
                    if last
                    else None,
...
def summarize(rows, probabilities, threshold, wide, payouts):
    ...
    for row, probability in zip(rows, probabilities, strict=True):
        ...
        if (
            row.popularity is not None
            and 4 <= row.popularity <= 10
            and row.win_odds
            and 5 <= row.win_odds <= 20
        ):
    ...
    for race_id, candidates in by_race.items():
        ...
        anchor = min(
            (row for row in field if row.win_odds),
            key=lambda row: row.win_odds,
            default=None,
        )
        ...
        if (
            ...
            or anchor.horse_no == rebound.horse_no
        ):
            continue
    ...
    return {
        "tickets": len(paid),
        "hits": sum(value > 0 for value in paid),
        "return_rate": round(sum(paid) / bet, 4) if bet else 0.0,
        "no_max_return_rate": round(sum(paid[:-1]) / (bet - 100), 4)
        if len(paid) > 1
        else 0.0,
        "no_top3_return_rate": round(sum(paid[:-3]) / (bet - 300), 4)
        if len(paid) > 3
        else 0.0,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=".workstate/jra-srb/jra-hidden-rebound-validation/report.json",
    )
    ...
    for c_value in CS:
        ...
        for threshold in PROBABILITY_THRESHOLDS:
            ...
            candidates.append(
                {
                    "name": f"rebound_c{c_value:g}_p{threshold:.2f}",
                    "q3": q3_summary,
                    "q4": q4_summary,
                    "h2": h2_summary,
                    "pass": all(
                        s["tickets"] >= 100
                        and s["return_rate"] >= 1
                        and s["no_max_return_rate"] >= 1
                        and s["no_top3_return_rate"] >= 1
                        for s in (q3_summary, q4_summary)
                    ),
                }
            )
    report = {
    ...
    print(
        json.dumps(
            {"candidate_count": len(candidates), "eligible": report["eligible"]},
            ensure_ascii=False,
        )
    )
