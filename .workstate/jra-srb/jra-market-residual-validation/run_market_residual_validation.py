    ...
    for runners in races.values():
        for rank, row in enumerate(
            sorted(
                runners,
                key=lambda item: (
                    -item["market_probability_normalized"],
                    item["horse_no"],
                ),
            ),
            1,
        ...
        for rank, row in enumerate(
            sorted(
                runners,
                key=lambda item: (-item["isotonic_probability"], item["horse_no"]),
            ),
            1,
        ):
            row["model_rank"] = rank
...
def select(rows: list[dict], config: tuple[int, float, float]) -> list[dict]:
    ...
    for row in rows:
        if (
            4 <= row["market_rank"] <= 6
            and row["model_rank"] <= rank_limit
            and row["win_odds"] <= odds_max
            and row["isotonic_probability"] - row["market_probability_normalized"]
            >= edge_min
        ):
            races[row["race_id"]].append(row)
    return [
        max(
            runners,
            key=lambda item: (
                item["isotonic_probability"] - item["market_probability_normalized"],
                item["isotonic_probability"],
            ),
        )
        for runners in races.values()
    ]
...
def metrics(items: list[dict]) -> dict:
    ...
    return {
        ...
        "over_30_rate": sum(item["win_odds"] > 30 for item in items) / len(items)
        if items
        else 0.0,
    }
...
def main() -> None:
    ...
    split = json.loads((root / "split-metadata.json").read_text(encoding="utf-8"))[
        "date_sets"
    ]
    ...
    for config in GRID:
        ...
        comparisons.append(
            {
                "config": {
                    "max_model_rank": config[0],
                    "max_win_odds": config[1],
                    "min_residual": config[2],
                },
                "metrics": result,
            }
        )
    ...
    selected = (
        max(
            viable,
            key=lambda item: (
                item["metrics"]["no_top3_roi"],
                item["metrics"]["no_max_roi"],
                item["metrics"]["roi"],
            ),
        )
        if viable
    ...
    external_metrics = (
        metrics(
            select(
                external,
                (
                    selected["config"]["max_model_rank"],
                    selected["config"]["max_win_odds"],
                    selected["config"]["min_residual"],
                ),
            )
        )
        if selected
    ...
    result = {
        "hypothesis": "市場4〜6番手だが履歴モデル上位の馬にだけ、市場に未反映の過去適性が残る",
        "status": "external_evaluated",
        "selected": selected,
        "external": external_metrics,
        "comparisons": comparisons,
    }
    out = Path(".workstate/jra-srb/jra-market-residual-validation")
    out.mkdir(parents=True, exist_ok=True)
    (out / "market-residual-validation.json").write_text(
